...
DIFFUSION_GEMMA = (
    REPO_ROOT / "vllm" / "model_executor" / "models" / "diffusion_gemma.py"
)
...
def test_initial_canvas_sync_is_gated_with_local_vocab_feature():
    tree = _module(DIFFUSION_GEMMA)
    cls = next(
        node
        for node in tree.body
        if isinstance(node, ast.ClassDef) and node.name == "DiffusionGemmaRequestStates"
    )
    init_canvas = next(
        node
        for node in cls.body
        if isinstance(node, ast.FunctionDef) and node.name == "init_canvas"
    )

    guarded_sync = [
        node
        for node in ast.walk(init_canvas)
        if isinstance(node, ast.If)
        and "_DIFFUSION_GEMMA_LOCAL_VOCAB_SAMPLER" in ast.unparse(node.test)
        and "_tp_broadcast_from_rank0" in ast.unparse(node)
    ]
    assert guarded_sync
...
def test_diffusion_gemma_local_logits_are_context_gated():
    ...
    assert (
        "_DIFFUSION_GEMMA_LOCAL_VOCAB_SAMPLER and self._allow_local_vocab_logits"
        in source
    )
